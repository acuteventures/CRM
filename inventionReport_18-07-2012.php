...
require_once('include/entryPoint.php');
	...
	if($_REQUEST['type']==1){
		...
		$sq1 .=" WHERE c_case_type.case_type_code='PS' AND c_case_status.order_no='100' AND users.first_name='".$_REQUEST['consultant_name']."'";
		...
		if($cnt>=1){
			while($R_sql= $db->fetchByAssoc($Q_sql)){
				...
				if($row_2 = $db->fetchByAssoc($result_2)){		
					...
					$str .=  "<tr>";
					$str .= "<td width='12%' valign='top' class='tabDetailViewDF'><input type='checkbox' name='invention_id' id='invention_id' value='".$row['id']."'>&nbsp;&nbsp;&nbsp;&nbsp;<a href='index.php?action=ajaxui#ajaxUILoc=index.php%3Fmodule%3DInv_Inventions%26action%3DDetailView%26record%3D".$R_sql['id']."' title='Click To See The Details Of Invantions'>".$R_sql['name']."</a></td>";
		...
		$sq1 .=" FROM inv_inventions LEFT JOIN inv_inventions_accounts_c ON inv_inventions_accounts_c.inv_invent9feaentions_idb = inv_inventions.id LEFT JOIN accounts ON accounts.id = inv_inventions_accounts_c.inv_inventd1acccounts_ida LEFT JOIN users ON users.id = accounts.assigned_user_id LEFT JOIN cases ON cases.invention_id = inv_inventions.id LEFT JOIN c_case_status ON cases.status = c_case_status.id LEFT JOIN c_case_type ON cases.type=c_case_type.id ";
		$sq1 .= " WHERE c_case_type.case_type_code='PP' AND c_case_status.order_no='100' AND users.first_name='".$_REQUEST['consultant_name']."' GROUP BY inv_inventions.id";
		$Q_sql = $db->query($sq1);
		$cnt = $db->getRowCount($Q_sql);
		$str = '';
		$str .=  "<table border='0'  cellpadding='0' cellspacing='{GRIDLINE}' width='100%' style='table-layout:fixed'>";
		...
		$sq1 .= " WHERE c_case_type.case_type_code='PP' AND users.first_name='".$_REQUEST['consultant_name']."'";
		$Q_sql = $db->query($sq1);
		$cnt = $db->getRowCount($Q_sql);
		$str = '';
		$str .=  "<table border='0'  cellpadding='0' cellspacing='{GRIDLINE}' width='100%' style='table-layout:fixed'>";
