...
<label for="website">{$label|strip_semicolon}:</label>
</td>
{counter name="fieldsUsed"}
...
<input type='text' name='{$fields.website.name}' id='{$fields.website.name}' size='30' 
maxlength='255' 
{if $displayView=='advanced_search'||$displayView=='basic_search'}value=''{else}value='http://'{/if} title='' tabindex='0'  >
{/if}
</tr>
{/capture}
{if $fieldsUsed > 0 }
...
{capture name="label" assign="label"}{sugar_translate label='LBL_PHONENO' module='Accounts'}{/capture}
...
<label for="client_source">{$label|strip_semicolon}:</label>
</td>
{counter name="fieldsUsed"}
...
<select name="{$fields.client_source.name}" 
id="{$fields.client_source.name}" 
title=''       
>
{if isset($fields.client_source.value) && $fields.client_source.value != ''}
{html_options options=$fields.client_source.options selected=$fields.client_source.value}
{else}
{html_options options=$fields.client_source.options selected=$fields.client_source.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.client_source.options }
{capture name="field_val"}{$fields.client_source.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.client_source.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.client_source.name}" 
id="{$fields.client_source.name}" 
title=''          
>
{if isset($fields.client_source.value) && $fields.client_source.value != ''}
...
<input
id="{$fields.client_source.name}-input"
name="{$fields.client_source.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.client_source.name}-image"></button><button type="button"
id="btn-clear-{$fields.client_source.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.client_source.name}-input', '{$fields.client_source.name}');sync_{$fields.client_source.name}()"><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
</span>
{literal}
<script>
...
{literal}
(function (){
var selectElem = document.getElementById("{/literal}{$fields.client_source.name}{literal}");
...
{/literal}
SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.client_source.name}-input');
SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.client_source.name}-image');
SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.client_source.name}');
{literal}
...
//global variable 
sync_{/literal}{$fields.client_source.name}{literal} = function(){
SyncToHidden();
}
...
for (i=0;i<selectElem.options.length;i++){
if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.client_source.name}-input{literal}'))
SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
}
}
YAHOO.util.Event.onAvailable("{/literal}{$fields.client_source.name}{literal}", syncFromHiddenToWidget);
{/literal}
...
<label for="password">{$label|strip_semicolon}:</label>
</td>
{counter name="fieldsUsed"}
...
addToValidate('EditView', 'accountfilter', 'enum', false,'{/literal}{sugar_translate label='' module='Accounts' for_js=true}{literal}' );
addToValidate('EditView', 'viewasfilter', 'enum', false,'{/literal}{sugar_translate label='' module='Accounts' for_js=true}{literal}' );
addToValidate('EditView', 'client_source', 'enum', false,'{/literal}{sugar_translate label='LBL_CLIENT_SOURCE' module='Accounts' for_js=true}{literal}' );
addToValidate('EditView', 'first_name', 'varchar', true,'{/literal}{sugar_translate label='LBL_FIRST_NAME' module='Accounts' for_js=true}{literal}' );
addToValidate('EditView', 'call_time', 'varchar', false,'{/literal}{sugar_translate label='LBL_CALL_TIME' module='Accounts' for_js=true}{literal}' );
addToValidate('EditView', 'last_name', 'varchar', false,'{/literal}{sugar_translate label='LBL_LAST_NAME' module='Accounts' for_js=true}{literal}' );
...
addToValidate('EditView', 'password', 'varchar', false,'{/literal}{sugar_translate label='LBL_PASSWORD' module='Accounts' for_js=true}{literal}' );
addToValidate('EditView', 'zohoid', 'varchar', false,'{/literal}{sugar_translate label='LBL_ZOHOID' module='Accounts' for_js=true}{literal}' );
addToValidate('EditView', 'mobile', 'varchar', false,'{/literal}{sugar_translate label='LBL_MOBILE' module='Accounts' for_js=true}{literal}' );
