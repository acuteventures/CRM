...
<input type='text' name='{$fields.case_number_advanced.name}' 
	...
	<td scope="row" nowrap="nowrap" width='10%' >
		...
		<label for='invention_name_advanced'>{sugar_translate label='LBL_INVENTION_NAME' module='Cases'}</label>
    	</td>
	<td  nowrap="nowrap" width='30%'>
			
<input type="text" name="{$fields.invention_name_advanced.name}"  class="sqsEnabled"   id="{$fields.invention_name_advanced.name}" size="" value="{$fields.invention_name_advanced.value}" title='' autocomplete="off"  >
<input type="hidden"  id="{$fields.invention_id_advanced.name}" value="{$fields.invention_id_advanced.value}">
<span class="id-ff multiple">
<button type="button" name="btn_clr_{$fields.invention_name_advanced.name}"   title="{$APP.LBL_CLEAR_BUTTON_TITLE}" class="button" onclick="this.form.{$fields.invention_name_advanced.name}.value = ''; this.form.{$fields.invention_id_advanced.name}.value = '';" value="{$APP.LBL_CLEAR_BUTTON_LABEL}">{sugar_getimage name="id-ff-clear" alt=$app_strings.LBL_ID_FF_CLEAR ext=".png" other_attributes=''}</button>
</span>
