...
***************************************************************/
	...
	if($btn_id == 'Contribute Case'){
		...
		if(contribute($id, $user_id, $db))
			$result .= "Successfully Contributed to this offcie action.";
		else{
			$result .= "Already contributed to this office action.";
		}
	}
	...
	function contribute($id, $user_id, $db){
		...
		if(!($row_cont['login_id'] == $user_id && $row_cont['case_id'] == $id)){
			...
			$cont_search->name = "Contribute Office Action ";
	...
	function caseHIstory($id, $user_id,$record_user, $flag = 0){
		...
		$caseHistory4->name = "Office Action  History - contribute ";
		$caseHistory4->parent_type = "oa_officeactions";
