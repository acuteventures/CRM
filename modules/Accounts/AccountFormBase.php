...
EOQ;
    ...
    function handleSave($prefix, $redirect = true, $useRequired = false) {
        ...
        if ($redirect) {
            ...
            $phone = new ph_phoneno();

            $phone->mark_deletedByAccountid($focus->id);
            $primary_no = $_REQUEST['primary0'];
            $count = $_REQUEST['count'];
            for ($i = 0; $i <= $count; $i++) {
                //echo $_REQUEST['phone_no'.$i];
                if ($_REQUEST['phone_no' . $i] != "") {

                    $phone = new ph_phoneno();

                    $ph_no = $_REQUEST['phone_no' . $i];
                    $phone->phone_no = $_REQUEST['phone_no' . $i];
                    $phone->primary_no = $primary_no;
                    $phone->account_id_c = $focus->id;
            ...
            $primary_email_idx = $_REQUEST['Accounts0emailAddressPrimaryFlag'];

            $count = $_REQUEST['emailCount'];
            global $db;
            $email_addr_del = "DELETE FROM  email_addresses,email_addr_bean_rel 
            ...
            $result = $db->query($email_addr_del);
            $email_bean_rel_del = "DELETE FROM email_addr_bean_rel 
                    WHERE email_addr_bean_rel.bean_id = '" . $focus->id . "'";
            $result = $db->query($email_bean_rel_del);
            ...
            for ($i = 0; $i <= $count; $i++) {
                ...
                if ($_REQUEST['Accounts0emailAddress' . $i] != "") {
                    
                    if ($i == $primary_email_idx)
                        $is_primary = true;
                    else
                        $is_primary = FALSE;
                    ...
                    for($j = 0;$j<=$i;$j++)
                    {
                        if($_REQUEST['Accounts0emailAddressOptOutFlag'][$j] == "Accounts0emailAddress".$i)
                        {
                            $is_opted_out = true;
                        }
                    }
                    ...
                    for($j = 0;$j<=$i;$j++)
                    {
                        if($_REQUEST['Accounts0emailAddressInvalidFlag'][$j] == "Accounts0emailAddress".$i)
                        {
                            $is_invalid = true;
                        }
                    }
                    ...
                    $emailAddr->addAddress($_REQUEST['Accounts0emailAddress' . $i], $is_primary,$replyTo,$is_invalid,$is_opted_out);
                    $emailAddr->save($focus->id, 'Accounts');
                }
            }
            /* *********GBR********** */
