		...
		elseif($_REQUEST['accountfilter_basic'][0] == 'all_leads'){
 		...
 		if($_REQUEST['viewasfilter_basic'][0] != "" && $_REQUEST['viewasfilter_basic'][0] != "all" ){
			$ret_array['where'] .= "WHERE (c_case_status.order_no < 20 AND accounts.id not in (select accounts.id from accounts LEFT JOIN cases ON cases.account_id = accounts.id LEFT JOIN c_case_status ON cases.status = c_case_status.id
			 where c_case_status.order_no >= 20 AND accounts.deleted = '0' AND cases.deleted = '0' AND c_case_status.deleted = '0') AND cases.deleted = '0' AND c_case_status.deleted = '0' AND accounts.assigned_user_id = '".$_REQUEST['viewasfilter_basic'][0]."') || (accounts.id not in(select cases.account_id from cases) AND accounts.assigned_user_id = '".$_REQUEST['viewasfilter_basic'][0]."') AND accounts.deleted = '0'";
 		}else{
			$ret_array['where'] = " where (c_case_status.order_no < 20 AND accounts.id not in (select accounts.id from accounts LEFT JOIN cases ON cases.account_id = accounts.id LEFT JOIN c_case_status ON cases.status = c_case_status.id
 where c_case_status.order_no >= 20 AND accounts.deleted = '0' AND cases.deleted = '0' AND c_case_status.deleted = '0') AND cases.deleted = '0' AND c_case_status.deleted = '0') || (accounts.id not in(select cases.account_id from cases)) AND accounts.deleted = '0'";
