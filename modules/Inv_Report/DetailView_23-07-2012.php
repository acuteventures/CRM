 ...
 ********************************************************************************/
 ...
 //customizing the detail for reports
	...
	$users	= array('' => '');
	$sql_for_users = "SELECT * FROM `users` WHERE deleted=0 ORDER BY first_name ASC";
    ...
    if($focus->name == 'Inventions with Search but no Patent'){
		...
		$sq1 .=" WHERE c_case_type.case_type_code='PS' AND c_case_status.order_no='100' AND users.id='".$current_user->id."' AND cases.deleted=0 AND inv_inventions.deleted=0 AND inv_inventions_accounts_c.deleted=0 AND accounts.deleted=0 AND users.deleted=0 AND c_case_type.deleted=0  AND c_case_status.deleted=0";
	...
	// Inventions with expired provisionals not claimed
		...
		$sq1 .=" FROM inv_inventions LEFT JOIN inv_inventions_accounts_c ON inv_inventions_accounts_c.inv_invent9feaentions_idb = inv_inventions.id LEFT JOIN accounts ON accounts.id = inv_inventions_accounts_c.inv_inventd1acccounts_ida LEFT JOIN users ON users.id = accounts.assigned_user_id LEFT JOIN cases ON cases.invention_id = inv_inventions.id LEFT JOIN c_case_status ON cases.status = c_case_status.id LEFT JOIN c_case_type ON cases.type=c_case_type.id ";
		$sq1 .= " WHERE c_case_type.case_type_code='PP' AND c_case_status.order_no='100' AND users.id='".$current_user->id."' AND cases.deleted=0 AND inv_inventions.deleted=0 AND inv_inventions_accounts_c.deleted=0 AND accounts.deleted=0 AND users.deleted=0 AND c_case_type.deleted=0  AND c_case_status.deleted=0 GROUP BY inv_inventions.id";
	...
	//Expiring Provisionals with No Full Patent in Progress
		...
		$sq1 .= " WHERE c_case_type.case_type_code='PP' AND users.id='".$current_user->id."' AND cases.deleted=0 AND inv_inventions.deleted=0 AND inv_inventions_accounts_c.deleted=0 AND accounts.deleted=0 AND users.deleted=0 AND c_case_type.deleted=0";
				...
				if($diff_days > 0 && $diff_days <=60){
					...
					if($count > 0){
						...
						if($flag_status == 0){
							...
							$str .= "<td width='12%' valign='top' class='tabDetailViewDF'><a href='index.php?module=Users&offset=7&stamp=1341809177067736200&return_module=Users&action=DetailView&record=".$row['user_id']."'>".$row['first_name']." ".$row['last_name']."</a></td>";
							$str .= "<td width='12%' valign='top' class='tabDetailViewDF'>testing 1</td>";
							$str .=  "</tr>";
						}
					}
					else{
						...
						$str .= "<td width='12%' valign='top' class='tabDetailViewDF'><a href='index.php?action=ajaxui#ajaxUILoc=index.php%3Fmodule%3DAccounts%26action%3DDetailView%26record%3D".$row['account_id']."'>".$row['client_name']."</a></td>";
						$str .= "<td width='12%' valign='top' class='tabDetailViewDF'><a href='index.php?module=Users&offset=7&stamp=1341809177067736200&return_module=Users&action=DetailView&record=".$row['user_id']."'>".$row['first_name']." ".$row['last_name']."</a></td>";
						$str .= "<td width='12%' valign='top' class='tabDetailViewDF'>testing 2</td>";
						$str .=  "</tr>";
					}
				}else{
					$str .=  "<tr>";
