...
by swapna
dt:-17-03-2012*/
	function casestatusAge(){
	
		global $db;
		$bef_date = "";
		$after_date = "";
				
				$case_sql="SELECT * FROM cases WHERE deleted=0 ";
				$res_case = $db->query($case_sql);
				while($row_case = $db->fetchByAssoc($res_case)){
				
				    $id = $row_case['id'];
					$status = $row_case['status'];
					$sql_case_audit_before = "SELECT * FROM cases_audit WHERE parent_id = '".$id."' 
							 AND after_value_string = '".$status."' ORDER BY  `date_created` DESC ";
					...
					$sql_case_status_age = " UPDATE cases SET case_status_age = '".$status_age."' WHERE id = '".$id."' AND deleted=0";
					$db->query($sql_case_status_age);					
					//by anuradha 6/8/2012					
					$sql_combined_case_subcase_age = " UPDATE c_s_d_case_subcase_dashlet SET case_subcase_status_age = '".$status_age."' WHERE case_subcase_id = '".$id."' AND deleted=0 ";					
					$db->query($sql_combined_case_subcase_age);					
					$GLOBALS['log']->info("sql_combined_case_subcase_age=".$sql_combined_case_subcase_age);								//end					
				}
			return true;
	}
/*job 7
by swapna
dt:-17-03-2012*/
	function subcasestatusAge(){
	
		global $db;
		$bef_date = "";
		$after_date = "";
				
				$subcase_sql="SELECT * FROM oa_officeactions WHERE deleted=0 ";
				$res_subcase = $db->query($subcase_sql);
				while($row_subcase = $db->fetchByAssoc($res_subcase)){
				
				    $id = $row_subcase['id'];
					$status = $row_subcase['subcase_status_id'];
					$sql_case_audit_before = "SELECT * FROM oa_officeactions_audit WHERE parent_id = '".$id."' 
							 AND after_value_string = '".$status."' ORDER BY  `date_created` DESC ";
					...
					$sql_case_status_age = " UPDATE oa_officeactions SET subcase_status_age = '".$status_age."' WHERE id = '".$id."' AND deleted=0 ";
					$db->query($sql_case_status_age);					
					//by anuradha 6/8/2012					
					$sql_combined_subcase_age = " UPDATE c_s_d_case_subcase_dashlet SET case_subcase_status_age = '".$status_age."' WHERE case_subcase_id = '".$id."' AND deleted=0 ";					
					$db->query($sql_combined_subcase_age);					
					$GLOBALS['log']->info("sql_combined_subcase_age=".$sql_combined_subcase_age);					
					//end	
				}
			return true;
	}
