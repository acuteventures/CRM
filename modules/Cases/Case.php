...
// Here our code start ADZ 28-Aug-2012	
	...
	function get_cases_list_for_CreditReportSearch($case_status_comp,$case_status_ab,$user_id)
	{
		...
		AND c_case_type.deleted =0 AND cases.deleted =0 AND c_case_status.deleted =0 AND cases.client_consultant_id='".$user_id."' GROUP BY cases.type";
		}else{
		$query = "SELECT c_case_type.name AS case_type_name, c_case_type.id AS case_type_id
		FROM c_case_type, cases, c_case_status
		WHERE cases.status = c_case_status.id AND ( cases.status = '".$case_status_comp."' OR  cases.status = '".$case_status_ab."') AND cases.type = c_case_type.id
		AND c_case_type.deleted =0 AND cases.deleted =0 AND c_case_status.deleted =0 GROUP BY cases.type";
		}
...
if(!empty($id)){
	...
	function get_cases_list_for_CreditReportSearch_default($case_status_comp,$case_status_ab,$user_id)
	{
	    ...
	    $ret_array = array();
		$query = "SELECT c_case_type.name AS case_type_name, c_case_type.id AS case_type_id
		FROM c_case_type, cases
		WHERE  ( cases.status = '".$case_status_comp."' OR  cases.status = '".$case_status_ab."') AND cases.type = c_case_type.id
		AND cases.client_consultant_id='".$user_id."' AND c_case_type.deleted =0 AND cases.deleted =0 GROUP BY cases.type";
	...
	function get_cases_consultant_Credits($case_type,$case_status_comp,$case_status_ab,$user_id,$From_date,$To_date )
	{
		...
		if($user_id != 'all'){
		$query = "SELECT   cases.client_consultant_id AS client_consultant_id, cases.id AS case_id,users.id As consultant_id,CONCAT(users.first_name,users.last_name) AS consultant FROM cases,users WHERE  cases.client_consultant_id=users.id AND cases.deleted=0 AND users.deleted=0  
        ...
        // Get the id and the name.
       $row = $this->db->fetchByAssoc($result);
		$user_array = array('client_consultant_id'=>$client_consultant_id,'case_id'=>$case_id,'consultant_id'=>$consultant_id,'consultant'=>$consultant);  
		return $user_array;
...
function getCaseSubjectAnsClientNameCredits_mine($case_type,$case_status_comp,$case_status_ab,$user_id,$From_date,$To_date,$consultant_id_Subject)
	{
	    global  $db,$current_user; 
		$consultant = array();
	    $casename = array();
		...
		if($user_id != 'all'){
			$query = "SELECT   cases.client_consultant_id AS client_consultant_id, cases.id AS case_id,CONCAT(users.first_name,users.last_name) AS consultant,cases.name AS 'case_Name',cases.credit_alloc_notes AS 'credit_allocation_notes',cases.credit_date AS credit_date,accounts.name AS'client_name' , accounts.id AS accounts_id  FROM cases,users,accounts , c_contribute WHERE  cases.client_consultant_id=users.id AND cases.deleted=0 AND users.deleted=0 AND c_contribute.deleted=0 AND accounts.deleted=0 AND 
			cases.account_id=accounts.id 
			AND ( cases.status = '".$case_status_comp."' OR  cases.status = '".$case_status_ab."')  AND cases.type='".$case_type."' AND (cases.client_consultant_id='".$user_id."' OR c_contribute.login_id = '".$user_id."') AND c_contribute.case_id=cases.id 
			AND cases.credit_date BETWEEN '".$From_date."' AND '".$To_date."'  GROUP BY cases.name ORDER BY cases.credit_date DESC";
		}
		else
		{
			$query = "SELECT   cases.client_consultant_id AS client_consultant_id, cases.id AS case_id,CONCAT(users.first_name,users.last_name) AS consultant,cases.name AS 'case_Name',cases.credit_alloc_notes AS 'credit_allocation_notes',cases.credit_date AS credit_date,accounts.name AS 'client_name' , accounts.id AS accounts_id  FROM cases,users,accounts , c_contribute WHERE  cases.client_consultant_id=users.id AND cases.deleted=0 AND users.deleted=0 AND c_contribute.deleted=0 AND accounts.deleted=0 AND 
			cases.account_id=accounts.id 
			AND ( cases.status = '".$case_status_comp."' OR  cases.status = '".$case_status_ab."')  AND cases.type='".$case_type."' AND c_contribute.case_id=cases.id  AND cases.credit_date BETWEEN '".$From_date."' AND '".$To_date."' AND (cases.client_consultant_id='".$consultant_id_Subject."'  OR c_contribute.login_id='".$consultant_id_Subject."') GROUP BY cases.name ORDER BY cases.credit_date DESC";
		}
...
$query = "SELECT   cases.client_consultant_id AS client_consultant_id, cases.id AS case_id,CONCAT(users.first_name,users.last_name) AS consultant,cases.name AS 'case_Name',cases.credit_alloc_notes AS 'credit_allocation_notes',cases.credit_date AS credit_date,accounts.name AS'client_name' , accounts.id AS accounts_id  FROM cases,users,accounts , c_contribute WHERE  cases.client_consultant_id=users.id AND cases.deleted=0 AND users.deleted=0 AND c_contribute.deleted=0 AND accounts.deleted=0 AND 
			cases.account_id=accounts.id 
			AND ( cases.status = '".$case_status_comp."' OR  cases.status = '".$case_status_ab."')  AND cases.type='".$case_type."' AND (cases.client_consultant_id='".$user_id."' OR c_contribute.login_id = '".$user_id."') AND c_contribute.case_id=cases.id AND users.id='".$consultant_id_Subject."'  
			AND cases.credit_date BETWEEN '".$From_date."' AND '".$To_date."'  GROUP BY cases.name ORDER BY cases.credit_date DESC";
}
else
{
$query = "SELECT   cases.client_consultant_id AS client_consultant_id, cases.id AS case_id,CONCAT(users.first_name,users.last_name) AS consultant,cases.name AS 'case_Name',cases.credit_alloc_notes AS 'credit_allocation_notes',cases.credit_date AS credit_date,accounts.name AS 'client_name' , accounts.id AS accounts_id  FROM cases,users,accounts , c_contribute WHERE  cases.client_consultant_id=users.id AND cases.deleted=0 AND users.deleted=0 AND c_contribute.deleted=0 AND accounts.deleted=0 AND 
cases.account_id=accounts.id 
AND ( cases.status = '".$case_status_comp."' OR  cases.status = '".$case_status_ab."')  AND cases.type='".$case_type."' AND c_contribute.case_id=cases.id  AND cases.credit_date BETWEEN '".$From_date."' AND '".$To_date."' AND cases.client_consultant_id='".$consultant_id_Subject."'  GROUP BY cases.name ORDER BY cases.credit_date DESC LIMIT 0,10";
...
$query = "SELECT cases.client_consultant_id AS client_consultant_id, cases.id AS case_id,cases.name AS 'case_name',cases.credit_alloc_notes AS 'credit_allocation_notes',cases.credit_date AS credit_date,accounts.name AS 'client_name' , accounts.id AS accounts_id  FROM cases,accounts , c_contribute WHERE   cases.account_id=accounts.id AND ( cases.status = '".$case_status_comp."' OR  cases.status = '".$case_status_ab."')  AND cases.type='".$case_type."' AND (cases.client_consultant_id='".$user_id."' OR c_contribute.login_id = '".$user_id."') AND c_contribute.case_id=cases.id  
 
AND cases.credit_date BETWEEN '".$From_date."' AND '".$To_date."'  AND cases.deleted=0 AND  c_contribute.deleted=0 AND c_contribute.deleted=0 GROUP BY cases.name  ORDER BY cases.credit_date DESC";
...
// Grand Total
	...
	function getGrandTotal($case_status_comp,$case_status_ab,$user_id,$From_date,$To_date ) {
				global $db,$current_user;
				
				 $query = "select SUM(c_contribute.credits) AS total_credits  from cases , c_contribute where c_contribute.login_id='".$user_id."'  AND c_contribute.case_id=cases.id and cases.credit_date BETWEEN '".$From_date."' AND '".$To_date."' AND cases.deleted=0 AND c_contribute.deleted=0 AND ( cases.status = '".$case_status_comp."' OR  cases.status = '".$case_status_ab."')";
...
// Subcase
	...
	function get_cases_subcaes($case_id)
	{
	    global  $db,$current_user; 
		...
		$query = "SELECT oa_officeactions.name AS subcase_name ,oa_officeactions.id AS subcase_id,cases.name AS case_name FROM oa_officeactions, oa_officeactions_cases_c, cases WHERE oa_officeactions.id = oa_officeactions_cases_c.oa_officeactions_casesoa_officeactions_idb AND cases.id = oa_officeactions_cases_c.oa_officeactions_casescases_ida AND cases.id='".$case_id."' AND oa_officeactions.deleted = 0 AND oa_officeactions_cases_c.deleted = 0 AND cases.deleted = 0 "; 


//echo $query;
        $result = $this->db->query($query, true, " Error filling in additional detail fields: ");
        while ($row = $db->fetchByAssoc($result)) {
				array_push($subcase_name, $row['subcase_name']);
				array_push($subcase_id, $row['subcase_id']);
   		 }
		$user_array = array('subcase_name'=>$subcase_name,'subcase_id'=>$subcase_id);  
		return $user_array;
	}	
