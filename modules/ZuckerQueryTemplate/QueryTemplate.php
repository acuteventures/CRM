...
class QueryTemplate extends ReportProviderBase {
	...
	// Author : Basudeba Rath, Date:15-Jun-2012.
		//case type
		if($_REQUEST['case_type'] != ""){
			$sql .= " AND cases.type='".$_REQUEST['case_type']."' ";
		}
		//case number
		if($_REQUEST['c_num'] != ""){
			$sql .= " AND cases.case_number='".$_REQUEST['c_num']."' ";
		}
		//consultant first name
		if($_REQUEST['first_name'] != ""){
			$sql .= " AND users.first_name LIKE '".$_REQUEST['first_name']."%' ";
		}
		//consultant last name
		if($_REQUEST['last_name'] != ""){
			$sql .= " AND users.last_name LIKE '".$_REQUEST['last_name']."%' ";
		}
		//client first name
		if($_REQUEST['client_first_name'] != ""){
			$sql .= " AND accounts.first_name LIKE '".$_REQUEST['client_first_name']."%' ";
		}
		//client last name
		if($_REQUEST['client_last_name'] != ""){
			$sql .= " AND accounts.last_name LIKE '".$_REQUEST['client_last_name']."%' ";
		}
		//credit date from
		//credit date to
		if($_REQUEST['date_from'] != "" && $_REQUEST['date_to'] != ""){
			
			$dt_from = date("Y-m-d",strtotime($_REQUEST['date_from']));
			$dt_to = date("Y-m-d",strtotime($_REQUEST['date_to']));
			$sql .= " AND cases.credit_date BETWEEN '".$dt_from."' AND '".$dt_to."' ";
		}

		$sql.= " ORDER BY cases.credit_date desc ";
