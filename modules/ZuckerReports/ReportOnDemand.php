...
if (!$is_scheduler) {
	...
	if (count($templates) > 0) {
		...
		//By preethi on 19-06-2012
		//for fetching the case types
		$caseTypes = new c_case_type();
		$casesList = $caseTypes->getAllCaseTypes();
		$cases_options_str	= get_select_options_with_id($casesList, $_REQUEST['case_type']);
		$cases_options_str = str_replace(array("\r\n","\n","\r"),"",$cases_options_str);
		$xtpl->assign("CASES_OPTIONS", $cases_options_str);
		
		//By preethi on 20-06-2012
		//sdded places holder for the filter fields
		$xtpl->assign("CASES_NUMBER", $_REQUEST['c_num']);
		$xtpl->assign("FIRST_NAME", $_REQUEST['first_name']);
		$xtpl->assign("LAST_NAME", $_REQUEST['last_name']);
		$xtpl->assign("CLIENT_FIRST_NAME", $_REQUEST['client_first_name']);
		$xtpl->assign("CLIENT_LAST_NAME", $_REQUEST['client_last_name']);
		$xtpl->assign("DATE_FROM", $_REQUEST['date_from']);
		$xtpl->assign("DATE_TO", $_REQUEST['date_to']);
		//End
		if (!empty($focus->id)) {
