...
function get_consultant_name()
{
    global  $db,$current_user; 
    $user_name = array();
    $user_key = array();
    $sql = "SELECT users.id,  users.first_name, users.last_name FROM users WHERE deleted ='0' AND status='Active'";
    $GLOBALS['log']->debug("get_consultant_name : ".$sql);
    $res = $db->query($sql);
    array_push($user_name, "all");
    array_push($user_key,"all");
    while ($row = $db->fetchByAssoc($res)) {
        if($row['id'] == $current_user->id){
            array_unshift($user_name, "Myself");#$row['first_name']."
            array_unshift($user_key,$current_user->id);
        }
        else{
            array_push($user_name, $row['first_name']." ".$row['last_name']);
            array_push($user_key,$row['id']);
        }
    ...
    $user_array = array_combine($user_key, $user_name);    
    return $user_array;
}

/*function get_consultant_name_drop_down()
{
    global  $db,$current_user; 
    $user_name = array();
    $user_key = array();
    $sql = "SELECT users.id,  users.first_name, users.last_name FROM users WHERE deleted ='0' AND status='Active' order by users.last_name";
    $GLOBALS['log']->debug("get_consultant_name : ".$sql);
    $res = $db->query($sql);
   // array_push($user_name, "Everyone");
    //array_push($user_key,"all");
    while ($row = $db->fetchByAssoc($res)) {
        if($row['id'] == $current_user->id){
            array_unshift($user_name, "Myself");#$row['first_name']." ".$row['last_name']
            array_unshift($user_key,$current_user->id);
        }
        else{
            array_push($user_name, $row['first_name']." ".$row['last_name']);
...
/*        if($row['id'] == $current_user->id){
		...
		array_push($user_name, $row['last_name']);
            array_push($user_key,$row['id']);
